...
def _(mo):
    mo.md(
        r"""
        # Phase 1: Quantitative Transkription - [Reichenauer Inkunabeln](https://regionalia.blb-karlsruhe.de/frontdoor/index/index/docId/22636)

        Printed german dataset from 15th and 16th Century (100 random pages).
        ---
        ---
        """
    )
...
def _(
    ...
    if hybrid_run_button.value:
        ...
        for hybrid_provider, hybrid_model_name in provider_models.items():
            ...
            # Create output directory
            hybrid_output_dir = f'temp/hybrid_zero_shot/{hybrid_provider}'
...
def _(
    ...
    if one_shot_llm_run_button.value:
        ...
        for one_shot_llm_provider, one_shot_llm_model_name in provider_models.items():
            ...
            # Create output directory
            one_shot_llm_output_dir = f'temp/one_shot_llm/{one_shot_llm_provider}'
            ...
            for one_shot_llm_doc_idx in mo.status.progress_bar(
                ...
                one_shot_llm_custom_messages = [
                    ...
                    {
                        "role": "assistant",
                        "content": "I'll follow this example for my transcription, maintaining the same style, characters, and line breaks."
                    },
                    {
                        "role": "user",
                        "content": [
...
def _(
    ...
    if one_shot_hybrid_run_button.value:
        ...
        for one_shot_provider, one_shot_model_name in provider_models.items():
            ...
            # Create output directory
            one_shot_output_dir = f'temp/one_shot_hybrid/{one_shot_provider}'
...
def _(
    ...
    if two_shot_llm_run_button.value:
        ...
        for two_shot_llm_provider, two_shot_llm_model_name in provider_models.items():
            ...
            # Create output directory
            two_shot_llm_output_dir = f'temp/two_shot_llm/{two_shot_llm_provider}'
            ...
            for two_shot_llm_doc_idx in mo.status.progress_bar(
                ...
                two_shot_llm_custom_messages = [
                    ...
                    {
                        "role": "assistant",
                        "content": "I'll follow both examples for my transcription, maintaining the same style, characters, and line breaks."
                    },
                ...
                two_shot_llm_result = process_document(
                    provider=f"{two_shot_llm_provider}_two_shot_llm",
            ...
            if two_shot_llm_all_results:
                ...
                # Calculate aggregate metrics
                calculate_aggregate_metrics(two_shot_llm_output_dir, f"{two_shot_llm_provider}_two_shot_llm")

                # Store for comparison
                two_shot_llm_results[f"{two_shot_llm_provider}_two_shot_llm"] = two_shot_llm_results_df

                # Add to comparison data
                two_shot_llm_comparison_data[f"{two_shot_llm_provider}_two_shot_llm"] = {
                ...
                # Show summary
                mo.md(f"**Summary for {two_shot_llm_provider} Two-Shot ({two_shot_llm_model_name}):**")
                mo.vstack([
                    mo.hstack([
                        mo.stat(f"{two_shot_llm_results_df['cer'].mean():.4f}", label="Average CER", bordered=True),
                        mo.stat(f"{two_shot_llm_results_df['wer'].mean():.4f}", label="Average WER", bordered=True),
                        mo.stat(f"{two_shot_llm_results_df['bwer'].mean():.4f}", label="Average BWER", bordered=True),
                        mo.stat(len(two_shot_llm_results_df), label="Documents", bordered=True)
                    ])
                ])
...
def _(
    ...
    if two_shot_hybrid_run_button.value:
        # Display evaluation in progress
        mo.md("## Two-Shot Hybrid Evaluation in Progress (MM-LLM + Transkribus)")
        ...
        if two_shot_examples.get("example1_image_path"):
            try:
                ...
                mo.md("✅ Successfully encoded example 1 image")
            except Exception as e:
                mo.md(f"⚠️ Failed to encode example 1 image: {str(e)}")
        ...
        if two_shot_examples.get("example2_image_path"):
            try:
                ...
                mo.md("✅ Successfully encoded example 2 image")
            except Exception as e:
                mo.md(f"⚠️ Failed to encode example 2 image: {str(e)}")
        ...
        mo.md(f"Found {len(two_shot_available_images)} images to process")
        ...
        mo.md(f"Found {len(two_shot_gt_files)} matching ground truth files")
        ...
        for two_shot_provider, two_shot_model_name in provider_models.items():
            mo.md(f"### Evaluating Two-Shot Hybrid {two_shot_provider.upper()} + Transkribus with {two_shot_model_name}")

            # Create output directory
            two_shot_output_dir = f'temp/two_shot_hybrid/{two_shot_provider}'
            ...
            for two_shot_doc_idx in mo.status.progress_bar(
                ...
                if two_shot_result['status'] == 'success':
                    mo.md(f"✅ **{two_shot_result['doc_id']}**: {two_shot_result['message']}")
                    two_shot_all_results.append(two_shot_result['metrics'])
                else:
                    mo.md(f"⚠️ **{two_shot_result['doc_id']}**: {two_shot_result['message']}")
            ...
            if two_shot_all_results:
                ...
                # Show summary
                mo.md(f"**Summary for {two_shot_provider} Two-Shot Hybrid ({two_shot_model_name} + Transkribus):**")
                mo.vstack([
                    mo.hstack([
                        mo.stat(f"{two_shot_results_df['cer'].mean():.4f}", label="Average CER", bordered=True),
                        mo.stat(f"{two_shot_results_df['wer'].mean():.4f}", label="Average WER", bordered=True),
                        mo.stat(f"{two_shot_results_df['bwer'].mean():.4f}", label="Average BWER", bordered=True),
                        mo.stat(len(two_shot_results_df), label="Documents", bordered=True)
                    ])
                ])
    ...
    return (
        ...
        two_shot_results,
        two_shot_results_df,
        two_shot_transkribus_dir,
        two_shot_transkribus_lines,
        two_shot_transkribus_path,
        two_shot_transkribus_text,
    )
