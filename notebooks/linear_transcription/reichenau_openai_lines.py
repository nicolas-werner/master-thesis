...
def _(
    ...
    if zero_shot_run_button.value:
        def create_zero_shot_messages(doc_id, image_path):
            ...
            return [
                {"role": "system", "content": system_prompt.value},
                {
                    "role": "user",
                    "content": [
                        # {"type": "text", "text": "Please transcribe this historical printed page (incunabulum) exactly as it appears. Preserve the original spelling, characters, and maintain line breaks exactly as in the image — each printed line must be one line in your transcription."},
                        {
                            "type": "image_url",
        ...
        def create_hybrid_messages(doc_id, image_path):
            ...
            return [
                {"role": "system", "content": system_prompt.value},
                {
                    "role": "user",
                    "content": [
                        {"type": "text", "text": f"The following is the output of a traditional OCR model from Transkribus. It is fine-tuned on medieval texts. It can help you transcribe the following page, but may also contain errors:\n\n{transkribus_text}"},
                        {
                            "type": "image_url",
                            "image_url": {"url": f"data:image/jpeg;base64,{image_base64}"}
                        }
                ...
                return [
                    ...
                    },
                    {
                        ...
                        "content": [
                            {
                                "type": "text",
                                "text": "Review the following example of a historical printed page (incunabulum) along with its correct transcription. This shows how to handle line breaks, abbreviations, and typographic features.\n\n**Use this example to learn how to transcribe historical documents — do not copy the example text directly.**"
                            },
                            {
                                "type": "image_url",
                ...
                return [
                    ...
                    },
                    {
                        ...
                        "content": [
                            {
                                "type": "text",
                                "text": "Review the following examples of a historical printed page (incunabulum) along with its correct transcription. This shows how to handle line breaks, abbreviations, and typographic features.\n\n**Use these examples to learn how to transcribe historical documents — do not copy the example text directly.**"
                            },
                            {
                                "type": "image_url",
...
def _():
    ...
    #         def create_two_shot_hybrid_openai_messages(doc_id, image_path):
    #             # Instead of getting Transkribus text, get OpenAI's two-shot transcription
    ...
    #                 print(f"✅ Using OpenAI two-shot transcription for {doc_id}")
    #             else:
    #                 # Fallback to Transkribus if needed
    #                 print(f"⚠️ No OpenAI transcription found for {doc_id}, falling back to Transkribus")
    #                 openai_transcription, _ = get_transkribus_text(
    #                     doc_id,
    #                     'results/linear_transcription/reichenau_inkunabeln/transkribus_10_test'
    #                 )
    ...
    #             image_dir='data/reichenau_10_test/images',
    #             base_output_dir='temp',
    #             create_messages=create_two_shot_hybrid_openai_messages,
    #             eval_type='two_shot_hybrid_openai',
    #             limit=limit_docs
    #         )
    #     else:
    #         print("⚠️ Could not load both examples for two-shot learning")
