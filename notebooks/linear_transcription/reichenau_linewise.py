...
def display_comparison_stats(os):
    ...
    def visualize_message(doc_id, line_id, line_image, line_idx, message_func, prompt_type="zero_shot", output_dir='debug_visualization'):
        """
        Visualize the message by saving the line image and prompt content separately
        Organizes output into subfolders by prompt type

        Args:
            doc_id: Document ID
            line_id: Line ID
            line_image: PIL Image of the target line
            line_idx: Index of the line in the document
            message_func: The message creation function to visualize
            prompt_type: Type of prompt (zero_shot, one_shot, hybrid, etc.)
        ...
        # Create prompt-type specific subfolder
        prompt_dir = os.path.join(output_dir, prompt_type)
        os.makedirs(prompt_dir, exist_ok=True)

        # Create document-specific subfolder to further organize output
        doc_dir = os.path.join(prompt_dir, doc_id)
        os.makedirs(doc_dir, exist_ok=True)

        # File prefix for this specific line
        prefix = f"line_{line_idx}"

        # Get the actual messages that would be sent to the API
        messages = message_func(doc_id, line_id, line_image, line_idx)

        # 1. Save the line image
        line_image.save(os.path.join(doc_dir, f"{prefix}_image.jpg"))

        # 2. Save the prompt content as text
        with open(os.path.join(doc_dir, f"{prefix}_prompt.txt"), 'w', encoding='utf-8') as f:
                ...
                if isinstance(content, list):
                    for item in content:
                        if 'text' in item:
                            ...
                            f.write(f"IMAGE: [Image data not shown]\n\n")
                else:
                    f.write(f"{content}\n\n")

        # 3. Save a simple visualization showing message structure
        plt.figure(figsize=(8, 4))
        plt.title(f"Document: {doc_id}, Line: {line_idx}, Prompt Type: {prompt_type}")
        ...
        # Save visualization
        plt.savefig(os.path.join(doc_dir, f"{prefix}_preview.png"), dpi=150, bbox_inches='tight')
        plt.close()

        # 4. Save the complete structure as JSON for reference
        # First create a serializable version (remove base64 data)
        ...
        with open(os.path.join(doc_dir, f"{prefix}_structure.json"), 'w', encoding='utf-8') as f:
    ...
    def get_example_content(num_examples=1):
        ...
        example_paths = [
            {
                "gt_path": 'data/reichenau_10_test/few-shot-samples/7474192.xml',
                "img_path": 'data/reichenau_10_test/few-shot-samples/7474192.jpg'
            },
            {
                "gt_path": 'data/reichenau_10_test/few-shot-samples/7764258.xml',
                "img_path": 'data/reichenau_10_test/few-shot-samples/7764258.jpg'
            }
        ]
...
def _(mo):
    mo.md(
        r"""
        # Phase 1: LINEWISE Quantitative Transkription - [Reichenau Inkunabeln]()
...
def _(mo):
    ...
    transkribus_df = evaluate_transkribus(
        ...
        output_dir='reichenau_temp_lines',
            ...
            messages = [
                {"role": "system", "content": system_prompt.value},
                {
                    ...
                    "content": [
                        ...
                        {"type": "text", "text": f"The following is the OCR output for this line. It may contain errors:\n{transkribus_line_text}"},
                    ]
                }
            ]
        ...
        if os.path.exists(example_img_path) and os.path.exists(example_xml_path):
            ...
            if gt_text:
                ...
                if os1_line_data['lines'] and len(os1_line_data['lines']) > 0:
                    os1_line = os1_line_data['lines'][1]
                        ...
                        messages = [
                            ...
                            },
                            {
                                ...
                                "content": [
                                    {
                                        "type": "text",
                                        "text": "Review the following example of a line from a historical printed document along with its correct transcription. This shows how to handle abbreviations and typographic features.\n\n**Use this example to learn how to transcribe historical documents — do not copy the example text directly.**"
                                    },
                                    {
                                        "type": "image_url",
    ...
    if one_shot_hybrid_run_button.value:
        ...
        if os.path.exists(example_img_path) and os.path.exists(example_xml_path):
            ...
            if osh_line_data['lines'] and len(osh_line_data['lines']) > 0:
                osh_line = osh_line_data['lines'][1]
                ...
                if osh_transkribus_lines and len(osh_transkribus_lines) > 0:
                    osh_line_transkribus_text = osh_transkribus_lines[1].get('text', '')
                    ...
                    messages = [
                        ...
                        },
                        {
                            ...
                            "content": [
                                ...
                                {
                                    "type": "text",
                                    "text": f"The following is TRANSKRIBUS OCR OUTPUT for this image. It can help you transcribe this line, but may also contain errors:\n\n{transkribus_line_text}"
                                }
                            ]
                        }
                    ]
...
def _():
    ...
    #                             {
    #                                 "type": "text",
    #                                 "text": "Review the following example of a line from a historical printed document. It includes both:\n\n(1) A raw OCR transcription from Transkribus\n(2) The correct ground truth transcription\n\nThis shows how to improve upon the OCR output and handle abbreviations or typographic features.\n\n**Use this example to learn how to improve OCR transcriptions — do not copy the example text directly.**"
    #                             },
    #                             {
    #                                 "type": "image_url",
    #                                 "image_url": {"url": f"data:image/jpeg;base64,{osh_line_image_base64}"}
    ...
    #                             {
    #                                 "type": "text",
    #                                 "text": "Now transcribe the following line from a historical printed document:"
    #                             },
    #                             {
    #                                 "type": "image_url",
    #                                 "image_url": {"url": f"data:image/jpeg;base64,{line_image_base64}"}
    ...
    #                             {
    #                                 "type": "text",
    #                                 "text": f"The following is the output of a traditional OCR model from Transkribus. It can help you transcribe this line, but may also contain errors:\n\n{transkribus_line_text}"
    #                             }
    #                         ]
    #                     }
    #                 ]
    ...
    #                 available_images = []
    #                 for f in os.listdir('data/reichenau_10_test/images'):
    ...
    #                                     line_messages = create_one_shot_hybrid_line_messages(
    #                                         doc_id=doc_id,
...
def _(
    ...
    if two_shot_run_button.value:
        ...
        ts_example1_img_path = 'data/reichenau_10_test/few-shot-samples/7474192.jpg'
        ts_example1_xml_path = 'data/reichenau_10_test/few-shot-samples/7474192.xml'

        ts_example2_img_path = 'data/reichenau_10_test/few-shot-samples/7764258.jpg'
        ts_example2_xml_path = 'data/reichenau_10_test/few-shot-samples/7764258.xml'
        ...
        if files_exist:
            ...
            if (ts_example1_line_data['lines'] and len(ts_example1_line_data['lines']) > 0 and
                ...
                ts_example1_line = ts_example1_line_data['lines'][1]
                        ...
                        return [
                            ...
                            },
                            {
                                ...
                                "content": [
                                    {
                                        "type": "text",
                                        "text": "Review the following examples of lines from historical printed documents along with their correct transcription. This shows how to handle abbreviations and typographic features.\n\n**Use these examples to learn how to transcribe historical documents — do not copy the example text directly.**"
                                    },
                                    {
                                        "type": "image_url",
                            ...
                            },
                            {
                                ...
                                "content": [
                                    {
                                        "type": "text",
                                        "text": "Here's a second example of a historical printed line along with its correct transcription:"
                                    },
                                    {
                                        "type": "image_url",
...
def _(
    ...
    if two_shot_hybrid_run_button.value:
        # Manually construct the examples with the correct paths
        tsh_example1_img_path = 'data/reichenau_10_test/few-shot-samples/7474192.jpg'
        tsh_example1_xml_path = 'data/reichenau_10_test/few-shot-samples/7474192.xml'
        tsh_example1_doc_id = '7474192'

        tsh_example2_img_path = 'data/reichenau_10_test/few-shot-samples/7764258.jpg'
        tsh_example2_xml_path = 'data/reichenau_10_test/few-shot-samples/7764258.xml'
        tsh_example2_doc_id = '7764258'
        ...
        if files_exist:
            ...
            if (tsh_example1_line_data['lines'] and len(tsh_example1_line_data['lines']) > 0 and
                ...
                # Get first line from first example
                tsh_example1_line = tsh_example1_line_data['lines'][1]
                ...
                tsh_example1_transkribus_path = find_file_for_id(
                    tsh_example1_doc_id,
                    'data/reichenau_10_test/few-shot-samples/transkribus',
                    ['.xml']
                )
                ...
                if tsh_example1_transkribus_path:
                    tsh_example1_transkribus_lines = extract_line_coords_from_xml(tsh_example1_transkribus_path)
                    if tsh_example1_transkribus_lines and len(tsh_example1_transkribus_lines) > 0:
                        tsh_example1_line_transkribus_text = tsh_example1_transkribus_lines[1].get('text', '')
                ...
                tsh_example2_transkribus_path = find_file_for_id(
                    tsh_example2_doc_id,
                    'data/reichenau_10_test/few-shot-samples/transkribus',
                    ['.xml']
                )
                ...
                if tsh_example2_transkribus_path:
                    tsh_example2_transkribus_lines = extract_line_coords_from_xml(tsh_example2_transkribus_path)
                ...
                # Create a closure with the examples captured
                def tsh_create_message_function(example1_image_base64, example1_text, example1_transkribus_text,
                                            example2_image_base64, example2_text, example2_transkribus_text):
                        ...
                        return [
                            ...
                            },
                            {
                                ...
                                "content": [
                                    ...
                                    {
                                        "type": "text",
                                        "text": f"The following is the TRANSKRIBUS OCR OUTPUT for the line. It can help you transcribe this line, but may also contain errors:\n\n{transkribus_line_text}"
                                    }
                                ]
                            }
                        ]
                    return create_two_shot_hybrid_line_messages

                # Create the function with captured examples
                tsh_message_creator = tsh_create_message_function(
                ...
                two_shot_hybrid_line_results = run_line_evaluation(
                    ...
                    transkribus_dir='results/linear_transcription/reichenau_inkunabeln/transkribus_10_test',
                    base_output_dir='reichenau_temp_lines',
                    create_line_messages=tsh_message_creator,
...
@app.cell
def _(create_cross_method_comparison, json, mo, os, pd, transkribus_df):
    def load_results_from_temp(eval_type):
        """Load results for a specific evaluation type from temp folder"""
        comparison_path = f"reichenau_temp_lines/{eval_type}_comparison.json"
        ...
        for provider in comparison_data.keys():
            ...
            provider_results_path = f"reichenau_temp_lines/{eval_type}/{provider}/{provider}_all_results.csv"
